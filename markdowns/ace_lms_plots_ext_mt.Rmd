...
```{r ref.label="rm_outliers_and_preproc", eval=BRT_PRINT}
```
...
```{r ref.label="rm_outliers_and_preproc", eval=BOX_PRINT}
```
...
```{r ref.label="rm_outliers_and_preproc", eval=FLANK_PRINT}
```
...
```{r ref.label="rm_outliers_and_preproc", eval=SAAT_PRINT}
```
...
```{r setting vars for span}
this_task = "SPATIALSPAN"
this_task_long = "Spatial span"
vars_of_interest = c("object_count_span.overall")
vars_of_interest_long = c("Overall span (obj count)") # nice version for printing in graphs
```
...
```{r ref.label="rm_outliers_and_preproc", eval=SPAN_PRINT}
```
...
```{r ref.label="rm_outliers_and_preproc", eval=STROOP_PRINT}
```
...
```{r setting vars for task switch}
...
```{r ref.label="rm_outliers_and_preproc", eval=SWITCH_PRINT}
```
...
```{r setting vars for tap n trace}
this_task = "TNT"
...
```{r ref.label="rm_outliers_and_preproc", eval=TNT_PRINT}
```
